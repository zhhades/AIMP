...
public interface HCNetSDK extends Library {
    ...
    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary("haikang\\HCNetSDK.dll", HCNetSDK.class);
    /*** 宏定义 ***/
    //常量
    ...
    public static final int MAX_LINKAGE_CHAN_NUM  = 16;  //报警联动的通道的最大数量
    public static final int MAX_CABINET_COUNT     = 8;    //最大支持机柜数量
    public static final int MAX_ID_LEN            = 48;
    public static final int MAX_PARKNO_LEN        = 16;
    public static final int MAX_ALARMREASON_LEN   = 32;
    public static final int MAX_UPGRADE_INFO_LEN  = 48; //获取升级文件匹配信息(模糊升级)
    public static final int MAX_CUSTOMDIR_LEN     = 32; //自定义目录长度
    public static final int MAX_LED_INFO_LEN      = 512;//LED内容长度
    public static final int MAX_VOICE_INFO_LEN    = 128;//语音播报内容长度
    public static final int MAX_LITLE_INFO_LEN    = 64; //纸票标题内容长度
    public static final int MAX_DEVNAME_LEN       = 32;      //设备名称最大长度
    public static final int MAX_LED_INFO          = 256;     //屏幕字体显示信息最大长度
    public static final int MAX_TIME_LEN          = 32;      //时间最大长度
    public static final int MAX_CARD_LEN          = 24;      //卡号最大长度
    public static final int MAX_OPERATORNAME_LEN  = 32;     //操作人员名称最大长度
    ...
    public static final int MAX_FIRE_ALARM_ZONE        = 12; //最大消防主机报警分区个数
    public static final int MAX_FIRE_ALARM_POINT_ZONE  = 32; //最大消防主机报警火点个数
...
//网络(NET_DVR_NETCFG_V30结构)
    ...
    public static final int NET_DVR_GET_GROUP_CFG = 2112;    //获取群组参数
    public static final int NET_DVR_SET_GROUP_CFG = 2113;    //设置群组参数
    public static final int NET_DVR_CAPTURE_FACE_INFO = 2510;    //采集人脸信息
    public static final int NET_DVR_GET_ACS_EVENT = 2514;//设备事件获取
    public static final int NET_DVR_GET_ACS_WORK_STATUS_V50 = 2180; //获取门禁主机工作状态(V50)
    ...
    public static final int NET_DVR_GET_ALARMHOST_EXTERNAL_DEVICE_STATE = 2050;//获取485外接设备状态
    ...
    public static final int COMM_ITS_GATE_VEHICLE = 0x3052; //出入口车辆抓拍数据
    ...
    public static final int COMM_FIREDETECTION_ALARM = 0x4991;  //温度报警上传
...
//智能设备类型
    ...
    public static class NET_DVR_PDC_ALRAM_INFO extends Structure { //通道录像参数配置
        ...
        public Pointer pXmlBuf; // XML报警信息指针
...
//NET_DVR_Login()参数结构
    ...
    @Structure.FieldOrder(value= {"sSerialNumber","byAlarmInPortNum","byAlarmOutPortNum","byDiskNum",
            "byDVRType","byChanNum","byStartChan","byAudioChanNum","byIPChanNum","byZeroChanNum",
            "byMainProto","bySubProto","bySupport","bySupport1","bySupport2","wDevType","bySupport3",
            "byMultiStreamProto","byStartDChan","byStartDTalkChan","byHighDChanNum",
            "bySupport4","byLanguageType","byVoiceInChanNum","byStartVoiceInChanNo","bySupport5","bySupport6"
    ...
    @Structure.FieldOrder(value= {"sDeviceAddress","byUseTransport","wPort","sUserName","sPassword"
            ,"cbLoginResult","pUser","bUseAsynLogin","byProxyType","byUseUTCTime","byLoginMode"
            ,"byHttps","iProxyID","byVerifyMode","byRes2"})
    ...
    //NET_DVR_Login_V40()参数
    @Structure.FieldOrder(value= {"struDeviceV30","bySupportLock","byRetryLoginTime","byPasswordLevel"
            ,"byRes1","dwSurplusLockTime","byCharEncodeType","bySupportDev5","bySupport","byLoginMode"
            ,"dwOEMCode","iResidualValidity","byResidualValidity","bySingleStartDTalkChan","bySingleDTalkChanNums",
            "byPassWordResetLevel","bySupportStreamEncrypt","byMarketType","byRes2"})
    ...
    //预览V40接口
    @Structure.FieldOrder(value= {"lChannel","dwStreamType","dwLinkMode","hPlayWnd","bBlocked","bPassbackRecord"
            ,"byPreviewMode","byStreamID","byProtoType","byRes1","byVideoCodingType","dwDisplayBufNum"
            ,"byNPQMode","byRes"})
    ...
    */
    ...
    public static class NET_DVR_VOLTAGE extends Structure {
        public int iPhaseAVol;            //A相电压
        public int iPhaseBVol;            //B相电压
        public int iPhaseCVol;            //C相电压
        public int iAveragePhaseVol;    //相电压平均值
    }

    public static class NET_DVR_LINE_VOLTAGE extends Structure {
        public int iLineVolAB;            //AB线电压
        public int iLineVolBC;            //BC线电压
        public int iLineVolCA;            //CA线电压
        public int iAverageLineVol;    //线电压平均值
    }


    //电流
    public static class NET_DVR_CURRENT extends Structure {
        public int iPhaseACurrent;            //A相电流
        public int iPhaseBCurrent;            //B相电流
        public int iPhaseCCurrent;            //C相电流
        public byte[] byRes = new byte[4];
    }

    //功率
    public static class NET_DVR_POWER extends Structure {
        public int iPhaseAPower;            //A相功率
        public int iPhaseBPower;            //B相功率
        public int iPhaseCPower;            //C相功率
        public int iSysTotalPower;            //系统总功率
    }

    //功率因数
    public static class NET_DVR_POWER_FACTOR extends Structure {
        public int iPhaseAPowerFactor;     //A相功率因数
        public int iPhaseBPowerFactor;     //B相功率因数
        public int iPhaseCPowerFactor;     //C相功率因数
        public int iTotalPowerFactor;      //总功率因数
    }

    //频率
    public static class NET_DVR_FREQUENCY extends Structure {
        public int iPhaseAFrequency;        //A相频率
        public int iPhaseBFrequency;        //B相频率
        public int iPhaseCFrequency;        //C相频率
        public byte[] byRes = new byte[4];
    }

    //负载率
    public static class NET_DVR_LOAD_FACTOR extends Structure {
        public int iPhaseALoadFactor;        //A相负载率
        public int iPhaseBLoadFactor;        //B相负载率
        public int iPhaseCLoadFactor;        //C相负载率
        public byte[] byRes = new byte[4];
    }

    //UPS状态
    public static class NET_DVR_ALARMHOST_UPS_STATE extends Structure {
        public NET_DVR_VOLTAGE        struInputVoltage;    //输入电压
        public NET_DVR_VOLTAGE        struBypassVoltage;    //旁路电压
        public NET_DVR_VOLTAGE        struOutputVoltage;    //输出电压
        public int iRectifierVol;            //整流器电压
        public int iInverterVol;            //逆变器电压

        public NET_DVR_CURRENT    struInputCurrent;    //输入电流
        public NET_DVR_CURRENT    struBypassCurrent;    //旁路电流
        public NET_DVR_CURRENT    struOutputCurrent;    //输出电流
        public int iInverterCurrent;        //逆变器电流

        public NET_DVR_FREQUENCY    struInputFrequency;    //输入频率
        public NET_DVR_FREQUENCY    struBypassFrequency;    //旁路频率
        public NET_DVR_FREQUENCY    struOutputFrequency;    //输出频率
        public int iInverterFre;            //逆变器频率

        public NET_DVR_POWER         struInputPower;    //输入功率
        public NET_DVR_POWER        struBypassPower;    //旁路功率
        public NET_DVR_POWER        struOutputPower;    //输出功率
        public NET_DVR_POWER        struComplexPower;    //复功率

        public int iNormalPower;            //标称功率
        public int iPowerFacter;            //功率因数

        public NET_DVR_LOAD_FACTOR    struBatteryLoadFactor;    //电池负载率
        public int iBatteryEstimated;        //电池备份时间(min)
        public int iBatteryTemperature;        //电池温度
        public int iBatteryVoltage;            //电池电压

        public byte byRectifierState;        //整流器运行状态/*1正常 2异常*/
        public byte byInverterState;        //逆变器运行状态/*1正常 2异常*/
        public byte byChargeState;        //充电器运行状态/*1正常 2异常*/
        public byte byBatteryState;        //电池运行状态/*1正常 2异常*/
        public byte byAutoBypassState;    //自动旁路状态/*1正常 2异常*/
        public byte[] byRes2 = new byte[247];
    }

    //开关电源状态
    public static class NET_DVR_ALARMHOST_SWITCH_POWER_SUPPLY_STATE extends Structure {
        public NET_DVR_VOLTAGE        struACVoltage;    //交流电压
        public int iDCConvertVoltage;    //DC转换器电压
        public NET_DVR_CURRENT        struACCurrent;    //交流电流
        public int iTotalCurrent;            //负载总电流
        public int iBattery1Temperature;    //电池1温度
        public int iBattery2Temperature;    //电池2温度
        public int iBattery1Current;        //电池1电流
        public int iBattery2Current;        //电池2电流
        public int iBattery3Current;        //电池3电流
        public int iBattery4Current;        //电池4电流
        public int iBatteryTestVoltage;    //电池测试电压
        public int iRectifierOutputVoltage;    //整流模块输出电压
        public int iRectifierOutputCurrent;    //整流模块输出电流
        public int iDCOutputVoltage;        //直流输出电压
        public byte[] byRes = new byte[432];
    }

    //气体探测器状态
    public static class NET_DVR_ALARMHOST_GAS_DETECTOR_STATE extends Structure {
        public int iSF6;            //SF6，单位ppm
        public int iFlow;           //流量，ml/min
        public int iTemperature;        //温度，单位摄氏度
        public int iO2;            //氧气，单位%
        public int iHumidity;        //湿度，单位%
        public byte[] byRes = new byte[492];
    }

    //温湿度传感器状态
    public static class NET_DVR_ALARMHOST_TEMP_HUMI_SENSOR_STATE extends Structure {
        public int iTemperature;    //温度，单位摄氏度
        public int iHumidity;        //湿度，单位%
        public byte[] byRes = new byte[504];
    }

    //空调状态
    public static class NET_DVR_ALARMHOST_AIRCONDITION_STATE extends Structure {
        public int iTemperature;    //温度，单位摄氏度
        public int iHumidity;        //湿度，单位%
        public byte byRunState;        //运行状态 1-正常，2-停机
        public byte[] byRes = new byte[503];
    }

    //电量表状态
    public static class NET_DVR_ALARMHOST_ELECTRICITY_STATE extends Structure {
        public int iDCVoltage;        //直流电压
        public int iDCCurrent;        //直流电流
        public NET_DVR_VOLTAGE        struPhaseVoltage;    //相电压
        public NET_DVR_LINE_VOLTAGE struLineVoltage;    //线电压
        public NET_DVR_CURRENT[]    struCurrent = new NET_DVR_CURRENT[4];        //电流，struCurrent[0]表示第1路, 以此类推
        public int iAverageCurrent;    //平均电流
        public int iNeutralCurrent;        //中线电流
        public NET_DVR_POWER[]    struActivePower = new NET_DVR_POWER[4];    //有功功率 struActivePower[0]表示第一路有功功率，以此类推
        public NET_DVR_POWER[]    struReactivePower = new NET_DVR_POWER[4];    //无功功率 struReactivePower[0]表示第一路无功功率，以此类推
        public NET_DVR_POWER[]    struApparentPower = new NET_DVR_POWER[4];    //视在功率 struApprentPower[0]表示第一路视在功率，以此类推
        public NET_DVR_POWER_FACTOR[]    struPowerFactor = new NET_DVR_POWER_FACTOR[4];    //功率因数，struPowerFactor[0]表示第一路功率因数，以此类推
        public NET_DVR_FREQUENCY        struFrequency;            //频率
        public byte[] byRes = new byte[128];
    }

    //变电器温显表状态
    public static class NET_DVR_ALARMHOST_TRANSFORMER_TEMP_STATE extends Structure {
        public int iPhaseATemperature;        //A相温度
        public int iPhaseBTemperature;        //B相温度
        public int iPhaseCTemperature;        //C相温度
        public int iPhaseDTemperature;        //D相温度
        public byte[] byRes = new byte[496];
    }

    //水位传感器
    public static class NET_DVR_ALARMHOST_WATERLEVEL_SENSOR extends Structure {
        public int iWaterLevel;        //水位
        public byte[] byRes = new byte[508];
    }

    //扬尘噪声传感器
    public static class NET_DVR_ALARMHOST_DUSTNOISE_SENSOR extends Structure {
        public int iDust;        //扬尘
        public int iNoise;       //噪声
        public int iPM25;        //PM2.5
        public byte[] byRes = new byte[500];
    }

    //环境采集仪
    public static class NET_DVR_ALARMHOST_ENVIRONMENTAL_LOGGER extends Structure {
        public int iAmmoniaNitrogen;    //氨氮
        public int iCOD;                //COD
        public int iPH;                //PH
        public int iOxygen;            //氧气
        public int iSulfurDioxide;    //二氧化硫
        public int iSoot;            //烟尘
        public int iFluoride;        //氟化物
        public int iPollutedWater;    //污水
        public int iTotalPhosphorus;    //总磷
        public int iExhaust;            //废气
        public int iNitrogenOxides;    //氮氧化物
        public int iFlueGasTemperature;//烟气温度
        public int iFlueGasPressure;    //烟气压力
        public int iDustThickness; /*粉尘浓度*/
        public int iAirCleanLevel; /*空气洁净度等级*/
        public int iPm10Thickness; /*PM10浓度*/
        public byte[] byRes = new byte[448];
    }

    //风速传感器状态
    public static class NET_DVR_WIND_SPEED_STATE extends Structure {
        public int iWindSpeed; /*风速*/
        public byte[] byRes = new byte[508];
    }

    //通用扩展输出模块状态（48路，每位表示1路，0表示关，1表示开）
    public static class NET_DVR_GENERATE_OUTPUT_STATE extends Structure {
        public byte byOutPutState1; /*output1-8*/
        public byte byOutPutState2; /*output9-16*/
        public byte byOutPutState3; /*output17-24*/
        public byte byOutPutState4; /*output25-32*/
        public byte byOutPutState5; /*output33-40*/
        public byte byOutPutState6; /*output41-48*/
        public byte[] Res = new byte[506];
    }

    //浸水传感器状态
    public static class NET_DVR_SOAK_STATE extends Structure {
        public int iWaterLogVal;      /* 泄露位置，100倍*/
        public int iLeakResist1;      /*第一路漏水电阻*/
        public int iTotolResist1;     /*第一路总电阻*/
        public int iLeakResist2;      /*第二路漏水电阻*/
        public int iTotolResist2;     /*第二路总电阻*/
        public byte byWaterLogAlarm;    /* 泄漏报警1：正常，2：告警*/
        public byte byLeakAlarm1;       /*第一路漏水告警1：正常，2：告警*/
        public byte byFaultAlarm1;      /*第一路故障告警1：正常，2：告警*/
        public byte byLeakAlarm2;       /*第二路漏水告警1：正常，2：告警*/
        public byte byFaultAlarm2;      /*第二路故障告警1：正常，2：告警*/
        public byte[] byRes = new byte[487];
    }

    //太阳能控制器状态
    public static class NET_DVR_SOLAR_POWER_STATE extends Structure {
        public int iChangerVolInput;          /*充电设备输入电压*/
        public int iChangerCurInput;          /*充电设备输入电流*/
        public int iChangerPwInput;           /*充电设备输入功率*/
        public int iChangerVolOutput;         /*充电设备输出电压*/
        public int iChangerCurOutput;         /*充电设备输出电流*/
        public int iChangerPwOutput;          /*充电设备输出功率*/
        public int iDischangerVolOutput;      /*放电设备输出电压*/
        public int iDischangerCurOutput;      /*放电设备输出电流*/
        public int iDischangerPwOutput;       /*放电设备输出功率*/
        public int iDevTemperatrue;           /*设备内温度*/
        public byte byBatteryVolState;          /*蓄电池电压状态1：正常，2：异常*/
        public byte byBatteryTmpState;          /*蓄电池温度状态1：正常，2：异常*/
        public byte byChangerVolInputState;     /*充电设备输入电压状态1：正常，2：异常*/
        public byte byChangerRunState;          /*充电设备运行状态1：正常，2：待机*/
        public byte byChangerChgState;          /*充电设备充电状态1：正常，2：异常*/
        public byte byBatteryVolFlt;            /*蓄电池电压故障1：正常，2：告警*/
        public byte byBatteryTmpFlt;            /*蓄电池温度异常1：正常，2：告警*/
        public byte byBatteryResistanceFlt;     /*蓄电池内阻异常1：正常，2：告警*/
        public byte byVolRcgFlt;                /*额定电压识别错误1：正常，2：告警*/
        public byte byChangerVolInputFlt;       /*充电设备输入电压异常1：正常，2：告警*/
        public byte byChangerMosShort;          /*充电MOS管短路1：正常，2：告警*/
        public byte byChangerAntiOrMosShort;    /*充电或防反MOS管短路1：正常，2：告警*/
        public byte byChangerAntiShort;         /*防反MOS管短路1：正常，2：告警*/
        public byte byInputOverCur;             /*输入过流1：正常，2：告警*/
        public byte byLoadOverCur;              /*负载过流1：正常，2：告警*/
        public byte byLoadShort;                /*负载短路1：正常，2：告警*/
        public byte byLoadMosShort;             /*负载MOS管短路1：正常，2：告警*/
        public byte byChangerFlt;               /*充电设备故障1：正常，2：告警*/
        public byte[] byRes = new byte[454];
    }

    //SF6报警主机状态
    public static class NET_DVR_SF6_ALARMHOST_STATE extends Structure {
        public int iSf6Thick1;    /*第一点SF6浓度*/
        public int iO2Thick1;     /*第一点氧气浓度*/
        public int iTemprature1;  /*第一点温度*/
        public int iHumidity1;    /*第一点湿度*/
        public int iSf6Thick2;    /*第二点SF6浓度*/
        public int iO2Thick2;     /*第二点氧气浓度*/
        public int iTemprature2;  /*第二点温度*/
        public int iHumidity2;    /*第三点湿度*/
        public int iSf6Thick3;    /*第三点SF6浓度*/
        public int iO2Thick3;     /*第三点氧气浓度*/
        public int iTemprature3;  /*第三点温度*/
        public int iHumidity3;    /*第三点湿度*/
        public int iSf6Thick4;    /*第四点SF6浓度*/
        public int iO2Thick4;     /*第四点氧气浓度*/
        public int iTemprature4;  /*第四点温度*/
        public int iHumidity4;    /*第四点湿度*/
        public int iSf6Thick5;    /*第五点SF6浓度*/
        public int iO2Thick5;     /*第五点氧气浓度*/
        public int iTemprature5;  /*第五点温度*/
        public int iHumidity5;    /*第五点湿度*/
        public int iSf6Thick6;    /*第六点SF6浓度*/
        public int iO2Thick6;     /*第六点氧气浓度*/
        public int iTemprature6;  /*第六点温度*/
        public int iHumidity6;    /*第六点湿度*/
        public int iSf6Thick7;    /*第七点SF6浓度*/
        public int iO2Thick7;     /*第七点氧气浓度*/
        public int iTemprature7;  /*第七点温度*/
        public int iHumidity7;    /*第七点湿度*/
        public int iSf6Thick8;    /*第八点SF6浓度*/
        public int iO2Thick8;     /*第八点氧气浓度*/
        public int iTemprature8;  /*第八点温度*/
        public int iHumidity8;    /*第八点湿度*/
        public int iSf6Thick9;    /*第九点SF6浓度*/
        public int iO2Thick9;     /*第九点氧气浓度*/
        public int iTemprature9;  /*第九点温度*/
        public int iHumidity9;    /*第九点湿度*/
        public int iSf6Thick10;   /*第十点SF6浓度*/
        public int iO2Thick10;    /*第十点氧气浓度*/
        public int iTemprature10; /*第十点温度*/
        public int iHumidity10;   /*第十点湿度*/
        public int iSf6Thick11;   /*第十一点SF6浓度*/
        public int iO2Thick11;    /*第十一点氧气浓度*/
        public int iTemprature11; /*第十一点温度*/
        public int iHumidity11;   /*第十一点湿度*/
        public int iSf6Thick12;   /*第十二点SF6浓度*/
        public int iO2Thick12;    /*第十二点氧气浓度*/
        public int iTemprature12; /*第十二点温度*/
        public int iHumidity12;   /*第十二点湿度*/
        public byte byFanState1;    /*风机状态1 1：运行，2：停止*/
        public byte byFanState2;    /*风机状态2 1：运行，2：停止*/
        public byte byFanState3;    /*风机状态3 1：运行，2：停止*/
        public byte byFanState4;    /*风机状态4 1：运行，2：停止*/
        public byte byFanState5;    /*风机状态5 1：运行，2：停止*/
        public byte byFanState6;    /*风机状态6 1：运行，2：停止*/
        public byte byFanState7;    /*风机状态7 1：运行，2：停止*/
        public byte byFanState8;    /*风机状态8 1：运行，2：停止*/
        public byte byFanState9;    /*风机状态9 1：运行，2：停止*/
        public byte byFanState10;   /*风机状态10 1：运行，2：停止*/
        public byte byFanState11;   /*风机状态11 1：运行，2：停止*/
        public byte byFanState12;   /*风机状态12 1：运行，2：停止*/
        public byte[] byRes = new byte[308];
    }

    //称重仪状态
    public static class NET_DVR_WEIGHT_STATE extends Structure {
        public int iWeight;   /*称重值,1000倍*/
        public byte[] byRes = new byte[508];
    }

    //气象采集系统状态
    public static class NET_DVR_WEATHER_STATION_STATE extends Structure {
        public int iAirTemperature;              /* 空气温度*/
        public int dwAirHumidity;              /* 空气湿度*/
        public int dwWindSpeed;                /* 风速*/
        public int dwWindDirection;            /* 风向*/
        public int dwIlluminationIntensity;    /* 光照强度*/
        public int dwCO2;                      /* co2浓度*/
        public int dwPM25;                     /* PM2.5浓度*/
        public int dwAirPressure;              /* 大气压力*/
        public int iSoilTemperature;             /* 土壤温度*/
        public int dwSoilHumidity;             /* 土壤湿度*/
        public int dwRainFall;                 /*降雨量*/
        public byte byIsRainSnow;               /* 雨雪情况，1-无雨雪，2-有雨雪*/
        public byte[] byRes = new byte[467];
    }

    //水质监测仪状态
    public static class NET_DVR_WATER_QLT_STATE extends Structure {
        public int dwTurbidity;    /* 浊度值*/
        public int dwChlorine;     /* 余氯值*/
        public int dwPH;           /* PH 值*/
        public byte[] byRes = new byte[500];
    }

    //燃气监测系统状态
    public static class NET_DVR_FUEL_GAS_DETE_STATE extends Structure {
        public int iNatrualGasThick;  /* 天然气浓度*/
        public byte[] byRes = new byte[508];
    }

    //消防主机设备
    public static class NET_DVR_FIRE_ALARM_STATUS extends Structure {
        public byte[] byFireAlarm = new byte[MAX_FIRE_ALARM_ZONE*MAX_FIRE_ALARM_POINT_ZONE];           //火点状态参数位，1-正常，2-告警
        public byte[] byRes = new byte[128];   //保留位
    }

    public static class NET_DVR_EXTERNAL_DEVICE_STATE_UNION extends Union {
        public NET_DVR_ALARMHOST_UPS_STATE struUpsState;        //UPS状态
        public NET_DVR_ALARMHOST_SWITCH_POWER_SUPPLY_STATE struSwitchPowerState;//开关电源状态
        public NET_DVR_ALARMHOST_GAS_DETECTOR_STATE struGasDetectorState;//气体检测系统状态
        public NET_DVR_ALARMHOST_TEMP_HUMI_SENSOR_STATE struTempHumiSensorState;//温湿度传感器状态
        public NET_DVR_ALARMHOST_AIRCONDITION_STATE struAirConditionState;//空调状态
        public NET_DVR_ALARMHOST_ELECTRICITY_STATE struElectricityState;//电量表状态
        public NET_DVR_ALARMHOST_TRANSFORMER_TEMP_STATE struTransformerTempState;    //变电器状态
        public NET_DVR_ALARMHOST_WATERLEVEL_SENSOR struWaterLevelSensor;//水位传感器
        public NET_DVR_ALARMHOST_DUSTNOISE_SENSOR struDustNoiseSensor;//扬尘噪声传感器
        public NET_DVR_ALARMHOST_ENVIRONMENTAL_LOGGER struEnvironmentLogger;//环境采集仪
        public NET_DVR_WIND_SPEED_STATE struWindSpeedState; //风速传感器状态
        public NET_DVR_GENERATE_OUTPUT_STATE struGenerateOutputState; //通用扩展输出模块状态（48路，每位表示1路）
        public NET_DVR_SOAK_STATE struSoakState; //浸水传感器状态
        public NET_DVR_SOLAR_POWER_STATE struSolarPowerState; //太阳能控制器状态
        public NET_DVR_SF6_ALARMHOST_STATE struSF6AlarmHostState; //SF6报警主机状态
        public NET_DVR_WEIGHT_STATE struWeightState; //称重仪状态
        public NET_DVR_WEATHER_STATION_STATE struWeatherStationState; //气象采集系统状态
        public NET_DVR_WATER_QLT_STATE struWaterQltState; //水质监测仪状态
        public NET_DVR_FUEL_GAS_DETE_STATE struFuelGasDeteState; //燃气监测系统状态
        public NET_DVR_FIRE_ALARM_STATUS struFireAlarmStatus; //消防主机设备状态
        public byte[] byRes = new byte[512];
    }

    public static class NET_DVR_ALARMHOST_EXTERNAL_DEVICE_STATE extends Structure {
        public int dwSize;
    ...
    public static final int MAX_ALARMHOSTKEYBOARD = 64; //网络报警主机最大键盘数
    public static final int NET_SDK_MAX_NET_USER_NUM = 64;  //网络用户

    public static class NET_DVR_SUBSYSTEM_PARAM_EX extends Structure {
        public int dwSize;
        public NET_DVR_SCHEDTIME[] struAlarmTime = new NET_DVR_SCHEDTIME[MAX_DAYS * MAX_TIMESEGMENT_V30/*8*/]; //布撤防时间
        public byte byAlarmInAdvance;            // 布撤防提前提示时间0~45分
        public byte[] byRes1 = new byte[3];
        public byte[] byJointAlarmIn = new byte[MAX_ALARMHOST_ALARMIN_NUM / 8];//按位，表示子系统关联的报警输入通道号，0-不加入，1-加入
        public byte[] byJointKeyboard = new byte[MAX_ALARMHOSTKEYBOARD/*64*/ / 8];//按位，表示支持的键盘号，0-不加入，1-加入
        public byte[] byJointOpetaterUser = new byte[MAX_KEYBOARD_USER_NUM/8];//按位表示，bit0表示键盘操作用户1 ，0-不关联 1-关联
        public NET_DVR_REMIND_TIME[] struAlarmRemindTime = new NET_DVR_REMIND_TIME[MAX_DAYS * MAX_TIMESEGMENT_V30/*8*/];//MAX_TIMESEGMENT_V30表示每天最多支持8个提醒
        public byte[] byJointNetUser = new byte[NET_SDK_MAX_NET_USER_NUM / 8];//关联网络用户（按位表示，bit0表示网络用户1；0-不关联 1-关联）
        public byte[] byRes2 = new byte[280];            // 保留字节
    }

    public static class NET_DVR_REMIND_TIME extends Structure {
        public byte byEnable;//是否启用本次提醒 0-不启用，1-启用
    ...
    public static class NET_DVR_ALARMHOST_MAIN_STATUS_V40 extends Structure {
        public int dwSize;
        public byte[] bySetupAlarmStatus = new byte[MAX_ALARMHOST_ALARMIN_NUM]; //防区布防状态，(最大支持512个防区查询)，0-对应防区处于撤防状态，1-对应防区处于布防状态
        public byte[] byAlarmInStatus = new byte[MAX_ALARMHOST_ALARMIN_NUM]; //防区报警状态，(最大支持512个防区查询)，0-对应防区当前无报警，1-对应防区当前有报警
        public byte[] byAlarmOutStatus = new byte[MAX_ALARMHOST_ALARMOUT_NUM]; //触发器状态，(最大支持512个触发器查询)，0-对应触发器无报警，1-对应触发器有报警
        public byte[] byBypassStatus = new byte[MAX_ALARMHOST_ALARMIN_NUM]; //旁路状态 数组下标表示0对应防区1 0-表示防区没有旁路 1-表示防区旁路
        public byte[] bySubSystemGuardStatus = new byte[MAX_ALARMHOST_SUBSYSTEM/*32*/]; //子系统布防状态,0-对应子系统处于撤防状态，1-对应子系统处于布防状态
        public byte[] byAlarmInFaultStatus = new byte[MAX_ALARMHOST_ALARMIN_NUM]; //防区故障状态，0-对应防区处于正常状态，1-对应防区处于故障状态
        public byte[] byAlarmInMemoryStatus = new byte[MAX_ALARMHOST_ALARMIN_NUM]; //防区报警记忆状态， 0-对应防区当前无报警，1-对应防区当前有报警
        public byte[] byAlarmInTamperStatus = new byte[MAX_ALARMHOST_ALARMIN_NUM]; //防区防拆状态，0-对应防区无报警，1-对应防区有报警
        public byte[] byEnableSubSystem = new byte[MAX_ALARMHOST_SUBSYSTEM/*32*/]; //子系统启用状态，0-无效，1-对应子系统未启用，2-对应子系统启用
        public byte[] bySubSystemGuardType = new byte[MAX_ALARMHOST_SUBSYSTEM]; //子系统布放状态，0-无效，1-外出布防，2-即时布防，3-在家布防
        public byte[] byRes = new byte[448]; //保留字节
    }
    ...
    */
    ...
    //出入口车辆抓拍数据上传
    public static class NET_ITS_GATE_VEHICLE extends Structure {
        public int dwSize;        //结构长度
        public int dwMatchNo;        //匹配序号,由(车辆序号,数据类型,车道号)组成匹配码 默认0
        public byte byGroupNum;    //图片组数量（一辆过车相机多次抓拍的数量，代表一组图片的总数，用于延时匹配数据）默认 1 抓拍类型下图片数据 控制后续接收延时操作
        public byte byPicNo;        //连拍的图片组上传图片序号（接收到图片组数量后，表示接收完成。
        //接收超时不足图片组数量时，根据需要保留或删除）
        public byte bySecondCam;    //是否第二相机抓拍（如远近景抓拍的远景相机，或前后抓拍的后相机，特殊项目中会用到）标示前后相机抓拍图片
        public byte byRes; //标记为预留
        public short wLaneid;            //车道号1～32（索引车道号，可以跳跃）
        public byte byCamLaneId;    //对应相机车道号1～16（相机配置的车道号，可以跳跃，可以相同）
        public byte byRes1;            //保留
        public byte[] byAlarmReason = new byte[MAX_ALARMREASON_LEN]; //自定义报警类型 默认为中文
        public short wBackList;    //标记为是否报警数据 0 表示为正常过车数据 1 表示禁止名单
        public short wSpeedLimit;        //限速上限（超速时有效）km/h
        public int dwChanIndex;        //通道号
        public NET_DVR_PLATE_INFO struPlateInfo = new NET_DVR_PLATE_INFO();     //车牌信息结构
        public NET_DVR_VEHICLE_INFO struVehicleInfo = new NET_DVR_VEHICLE_INFO();    //车辆信息  含速度
        public byte[] byMonitoringSiteID = new byte[MAX_ID_LEN];        //监测点编号
        public byte[] byDeviceID = new byte[MAX_ID_LEN];                //设备编号
        public byte byDir;            //监测方向，0-其它 1 入场，2 出场
        public byte byDetectType;    //检测方式, 0-其他 1-地感触发，2-视频触发，3-多帧识别，4-雷达触发
        public byte[] byRes2 = new byte[2];
        public byte[] byCardNo = new byte[MAX_ID_LEN]; // 卡号
        public int dwPicNum;        //图片数量（与picGroupNum不同，代表本条信息附带的图片数量，图片信息由struVehicleInfoEx定义
        public NET_ITS_PICTURE_INFO[] struPicInfo = new NET_ITS_PICTURE_INFO[4];         //图片信息,单张回调，最多4张图，由序号区分
        //2013-11-19 出入口车辆抓拍数据上传（监听方式）新增参数
        public byte[] bySwipeTime = new byte[MAX_TIME_LEN];//增加刷卡时间  时间格式为yyyymmddhh24missfff
        public byte[] byRes3 = new byte[224];
    }
    ...
    */
    ...
    public static final int MAX_ALARMHOST_SUBSYSTEM  = 32;//报警主机最大子系统数
    ...
    public static class NET_DVR_THERMOMETRY_UPLOAD extends Structure {
        ...
        public byte byFaceSnapThermometryEnabled;//人脸抓拍测温使能 1-开启 0-关闭
        public byte[] byRes2 = new byte[2];
        public int dwChan; //通道号，查询条件中通道号为0xffffffff时该字段生效
        public NET_VCA_RECT struFaceRect = new NET_VCA_RECT();      //人脸子图区域
        public int dwTimestamp;//DSP时间戳
...
//2007-11-30 V211支持以下接口 //11
    ...
    boolean NET_DVR_GetLocalIP(BYTE_TWODIM[] strIParray, IntByReference pValidNum, boolean pEnableBind);
