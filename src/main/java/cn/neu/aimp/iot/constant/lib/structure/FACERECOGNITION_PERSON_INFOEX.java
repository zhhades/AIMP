...
public class FACERECOGNITION_PERSON_INFOEX extends NetSDKLib.SdkStructure {
    public byte[]    szPersonName = new byte[NetSDKLib.NET_MAX_PERSON_NAME_LEN];    // 姓名
    ...
    public byte[]    szID = new byte[NetSDKLib.NET_MAX_PERSON_ID_LEN];    // 人员唯一标示(身份证号码,工号,或其他编号)
    public short    wFacePicNum;    // 图片张数
    public Net_PIC_INFO[]    szFacePicInfo = new Net_PIC_INFO[NetSDKLib.NET_MAX_PERSON_IMAGE_NUM];    // 当前人员对应的图片信息
    ...
    public byte[]    szProvince = new byte[NetSDKLib.NET_MAX_PROVINCE_NAME_LEN];    // 省份
    public byte[]    szCity = new byte[NetSDKLib.NET_MAX_CITY_NAME_LEN];    // 城市
    public byte[]    szUID = new byte[NetSDKLib.NET_MAX_PERSON_ID_LEN];    // 人员唯一标识符,首次由服务端生成,区别于ID字段
    public byte[]    szCountry = new byte[NetSDKLib.NET_COUNTRY_LENGTH];    // 国籍,符合ISO3166规范
    public byte    byIsCustomType;    // 人员类型是否为自定义: 0 使用Type规定的类型 1 自定义,使用szCustomType字段
    public byte[]    szCustomType = new byte[NetSDKLib.NET_COMMON_STRING_16];    // 人员自定义类型
    public byte[]    szComment = new byte[NetSDKLib.NET_COMMENT_LENGTH];    // 备注信息
    public byte[]    szGroupID = new byte[NetSDKLib.NET_GROUPID_LENGTH];    // 人员所属组ID
    public byte[]    szGroupName = new byte[NetSDKLib.NET_GROUPNAME_LENGTH];    // 人员所属组名, 用户自己申请内存的情况时,
    public int    emEmotion;    // 表情
    public byte[]    szHomeAddress = new byte[NetSDKLib.NET_COMMON_STRING_128];    // 注册人员家庭地址
    ...
    public int[]    emEmotions = new int[NetSDKLib.NET_MAX_FACEDETECT_FEATURE_NUM];    // 人脸特征数组,与 byFeatureValidNum 结合使用 设置查询条件的时候使用
    public int    nCustomPersonInfoNum;    // 注册人员信息扩展个数
    public CUSTOM_PERSON_INFO[]    szCustomPersonInfo = new CUSTOM_PERSON_INFO[NetSDKLib.NET_MAX_CUSTOM_PERSON_INFO_NUM];    //注册人员信息扩展
